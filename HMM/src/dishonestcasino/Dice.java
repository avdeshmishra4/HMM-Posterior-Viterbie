...
public class Dice {
	...
	//	E:/SPRING2014/bioinformatics-course/assignHMM/seqOut.txt
	//	E:/SPRING2014/bioinformatics-course/assignHMM/AccuracyOut.txt
		...
		String output = scanner.nextLine(); // To read input file name
													// from
													// the console
		System.out.print("Provide file name to write accuracy" + '\n');
		String accuracyWriter = scanner.nextLine();		
		
		File ipFile = new File(output);
		File accuracyFile = new File(accuracyWriter);
		
					
		BufferReaderAndWriter brw = new BufferReaderAndWriter();											// create object of BufferReaderAndWriter
		BufferedWriter bw = brw.getWriter(ipFile);															// get the BufferedWriter from BufferReaderAndWriter to write in output file
		BufferReaderAndWriter accuracyBuffer = new BufferReaderAndWriter();
		BufferedWriter writeAccuracy = accuracyBuffer.getWriter(accuracyFile);
		...
		for (int i = 0; i < 20; i++) {																		// loop to create 20 different sequences with different transition probability from fair to loaded and loaded to fare
			...
			vp.generateViterbiMatrixAndPointerMatrix(actualSequence, observations, hmm, bw, stateSymbols, writeAccuracy);	// creates the viterbi matrix, viterbi pointer matrix and calls viterbiTraceBack method to predict the sequence of states 
			...
			pp.backwardAlgorithm(actualSequence, observations, hmm, bw,										// calls posterior backwardAlgorithm to predict the sequence of states using Posterior decoding	
					stateSymbols, forwardMatrix, writeAccuracy);
